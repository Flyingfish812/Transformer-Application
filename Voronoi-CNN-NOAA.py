...
# X_ki是180*360的表格，每一格存放两个数据：[距离最近的探测值,该点是不是探测器的点]
# y_ki是180*360的表格，每一格存放一个数据：该点的真实值
...
# 对于1040份样本，探测器数量有5种规格，摆放方式有5种，分别训练填充数据
for ki in tqdm(range(sen_num_kind)):
    ...
    for va in range(sen_num_var):
        ...
        for t in tqdm(range(1040)):
            ...
            # 每张图确定探测器的位置，处于横向第几格(0,179)，纵向第几格(0,359)
